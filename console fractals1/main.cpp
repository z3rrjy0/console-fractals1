...
float returnValueCustomEquation(const float& x) { // Make slopeCustomEquation return the first derivative of this func
  ...
  // return x % int(amplification); // (x must be an integer)
  float toReturn = 0;
...
char slopeEquation(std::vector<float> equation, const float& x) { // Still developing
  ...
  for (unsigned int i = 1; i < eqS;)     // Take
    equation[i - 1] = equation[i] * i++; // First
  equation.erase(equation.end() - 1);    // Derivative
...
char slopeCustomEquation(const float& x) { // Returns the value of the first derivative (constant function)
...
void plotGraph(const std::vector<float>& equation, const std::map<float, float>& mapEquation, const signed char& which) { // Can save the first derivative from the beginning
  ...
  for (int i = 0; i < csbi.dwSize.X; i++) {
    ...
    drawSpot(i, origin.Y - int(tmpResult), lineType, colours.at("lblue")); // The graph (slash/hyphen/pipe could get to be asterisk)
  }
  if (origin.X >= csbi.dwSize.X || origin.Y >= csbi.dwSize.Y) // If origin outside the window draw an 'X' as close as you can
    drawSpot(max(min(origin.X, csbi.dwSize.X - 1), 1), max(min(origin.Y, csbi.dwSize.Y - 1), 1), 'X', colours.at("red"));
  else drawSpot(origin.X, origin.Y, 'O', colours.at("red")); // Else draw it as an 'O'
...
void promptData() {
  ...
  std::cout << "Previous origin in X was " << origin.X; // (half.X - origin.X) / amplification; // With the cam pos
  std::cout << "\nOrigin in X = "; std::cin >> toChange;
  if (toChange != "x") origin.X = stoi(toChange); // half.X - stoi(toChange) * amplification; // with the cam pos
  std::cout << "Previous origin in Y was " << origin.Y; // (half.Y - origin.Y) / amplification; // With the cam pos
  std::cout << "\nOrigin in Y = "; std::cin >> toChange;
  if (toChange != "x") origin.Y = stoi(toChange); // half.Y - stoi(toChange) * amplification; // With the cam pos
...
void HSVtoRGB(float HSV[3], float(&colour)[3]) {
...
std::complex<long double> newtonPPrime(std::complex<long double>& z) {
  ...
  // z *= { 1/2., 0 }; // Alternative generalized
}

unsigned int newton(const std::complex<long double>& c, float(&colour)[3]) { // returns integer
  ...
  unsigned int i = 0; // Initialized before to use it later
...
float collatz(const std::complex<long double>& c) {
  ...
  return float(((2.L + 7.L * c - (2.L + 5.L * c) * cos(pi * c)) / 4.L).real()); // The real part of (2 + 7c - (2 + 5c)*cos(pi*c)) / 4
...
unsigned int fractal(std::complex<long double>& c, float(&colour)[3]) { // Should return the iteration colour
  c /= amplification;

  // (colour only applies for Zn+1 = z^3 + 1)
  // return newton(c, colour); //int toReturn = colour[0]; HSVtoRGB(colour, colour); return toReturn;
  // (commented but working)

  unsigned int tmpResult = mandelbrot(c); // Also Julia sets (update for a better performance)
...
const float colourR(const int& i) { // Blue
  return i / 3.f;
}

const float colourG(const int& i) { // Pink
  return (255 + i) / 3.f;
}

const float colourB(const int& i) { // Yellow
  return (510 + i) / 3.f;
}
...
void plotFractal(const bool saveAsImage) {
  if (saveAsImage) {
    goto afterBegin;
  begin:;
    // plotFractal(false); // cls(); if showing percentage
  afterBegin:;
    std::stringstream ss;
...
#pragma endregion
    ...
    if (showCoordAndOrigin) {
      image.draw_line(origin.X, 0, origin.X, fullscreen.Y, red); // Y axis
      image.draw_line(0, origin.Y, fullscreen.X, origin.Y, red); // X axis
      ...
      else image.draw_circle(origin.X, origin.Y, 3, red); // These two lines for the center spot
    }
    ...
    COORD halfScreen;
  before:; // Floating window settings
         if (isVirtualKeyPressed(VK_LEFT))  origin.X += sens; // UP
    else if (isVirtualKeyPressed(VK_RIGHT)) origin.X -= sens;
    else if (isVirtualKeyPressed(VK_UP))    origin.Y += SHORT(sens * 2.2f); // without 2.2f
    else if (isVirtualKeyPressed(VK_DOWN))  origin.Y -= SHORT(sens * 2.2f); // without 2.2f
  ...
  for (SHORT i = 0; i < csbi.dwSize.Y; i++) {
    for (SHORT j = 0; j < csbi.dwSize.X; j++) {
      ...
      */ // Not the best part of the code to std::cout anything...
      ...
      if (saveAsImage) { // Doesn't draw origin nor axis
  ...
  if (saveAsImage) { // Should move after the origin display
...
int main() {
  ...
  bool saveAsImage = false; // Still only for fratals
...
skipTest:;
  ...
  cls(); // getWindowSize();
  origin = csbi.dwSize;
  origin.X /= 2; // Initial position of the origin
  origin.Y /= 2; // is the center of the console
  
  // origin.X = fullscreen.X / 2; // Initial position of the origin is
  // origin.Y = fullscreen.Y / 2; // the center of the floating window

  int sens; // Arrow sensibility
  long double sensAmp; // Amplification sensibility
  ...
  while (!isVirtualKeyPressed(VK_ESCAPE)) { // Console settings
    if (isVirtualKeyPressed(VK_LEFT)) origin.X += sens; // UP (not the key)
    else if (isVirtualKeyPressed(VK_RIGHT)) origin.X -= sens;
    else if (isVirtualKeyPressed(VK_UP))    origin.Y += SHORT(sens / 2.2f); // Not exactly 2.2 but the console
    else if (isVirtualKeyPressed(VK_DOWN))  origin.Y -= SHORT(sens / 2.2f); // characters aren't squares
    ...
    else if (isVirtualKeyPressed(VK_F5)) { getWindowSize(); origin = csbi.dwSize; origin.X /= 2; origin.Y /= 2; amplification = 20; } // Reset position and amplification
    else if (isVirtualKeyPressed(VK_F6)) promptData();
    else if (isVirtualKeyPressed(VK_F1) && useFr) saveAsImage = true;
    else if (isVirtualKeyPressed(VK_F11)) max_iterations--;
    else if (isVirtualKeyPressed(VK_F12)) max_iterations++;
    else continue; // Only keep executing the function if some of the if statements were triggered, else restart the loop
    // getWindowSize();
